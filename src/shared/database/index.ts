...
export interface IBaseEntityService<Entity, InputType> {
	...
	remove(id: string, options?: RemoveOptions): Promise<boolean>
...
export const BaseEntityService = <Entity>(entityRef: Entity): any => {
	abstract class BaseEntityServiceHost
		...
		async remove(id: string, options?: RemoveOptions): Promise<boolean> {
...
export const importCore = TypeOrmModule.forFeature(coreEntities)

export const normalizeUrl = (url: string) => url.replace('http://', '').replace('https://', '')
